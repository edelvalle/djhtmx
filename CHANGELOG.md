...
- **A non-optional `user` annotation is now enforced as a login requirement**: a component that declares `user: Annotated[User, Field(exclude=True)]` (instead of the optional annotation inherited from `HtmxComponent`) refuses to be built without a logged-in user.  A new `model_validator` on `HtmxComponent` reads the component's own `user` annotation and, when it does not admit `None`, rejects a missing, anonymous or primary-key-less user by raising the new `djhtmx.exceptions.LoginRequired` -- before any handler or render runs.  `CommandProcessor.process` answers requests to the `/_htmx/` endpoints with a `Redirect` to `settings.LOGIN_URL`, and `djhtmx.middleware` gained a `process_exception` hook that answers a full page render with a redirect to the login page carrying the requested page as `next`.
