...
### `SSEHeartbeat` and `get_sse_heartbeat_topic`
...
Components subscribe to heartbeat events by adding a normal `SSESubscription`:

```python
SSESubscription(SSEHeartbeat, get_sse_heartbeat_topic(self, 60))
```

`get_sse_heartbeat_topic(component, pace)` creates a session-local heartbeat topic. The topic is tied to the target component's current djhtmx session, so heartbeat events are generated only for components on the live page connection. Components must be bound to a djhtmx session before requesting heartbeat topics.
